...
###### We will now construct the basic algorithm in how we compute SVD and the matrices defined above. First we will compute the singula values $\sigma_{i}$ for our matrix $\Sigma$. Given a matrix $A$, the singular values are given by the eigenvalue of $AA^{T}$. We will begin an example of this with the defined matrix below,
$$\begin{bmatrix}
3 & 2 & 2\\
2 & 3 & -2
\end{bmatrix}$$
